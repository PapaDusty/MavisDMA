...
local MONTSERRAT_ASSET = "rbxassetid://11702779517"

local function ThemeFont(weight)
	local ok, font = pcall(function()
		return Font.new(MONTSERRAT_ASSET, weight, Enum.FontStyle.Normal)
	end)
	if ok then
		print(("[MavisDMA] Font loaded OK: Montserrat weight=%s family=%s"):format(tostring(weight), tostring(font.Family)))
		return font
	end
	warn("[MavisDMA] Failed to load Montserrat font (" .. tostring(font) .. "), falling back to Gotham/Montserrat enum")
...
local Theme = {
	...
	Font = ThemeFont(Enum.FontWeight.Medium),
	FontMedium = ThemeFont(Enum.FontWeight.Medium),
	FontSemibold = ThemeFont(Enum.FontWeight.SemiBold),
	FontBold = ThemeFont(Enum.FontWeight.Bold),
}
...
function Library:CreateWindow(config)
	...
	local GearButton = New("TextButton", {
		...
		Position = UDim2.new(0, 244, 0.5, -17),
		BackgroundColor3 = Theme.Element,
		Text = "",
...
function Library:AddTab(config)
	...
	local function RelayoutColumns()
		local w = Columns.AbsoluteSize.X
		if w <= 0 then
			return
		end
		local colWidth = math.floor((w - COLUMN_GAP) / 2)
		ColumnFrames[1].Size = UDim2.new(0, colWidth, 0, 0)
		ColumnFrames[1].Position = UDim2.new(0, 0, 0, 0)
		ColumnFrames[2].Size = UDim2.new(0, colWidth, 0, 0)
		ColumnFrames[2].Position = UDim2.new(0, colWidth + COLUMN_GAP, 0, 0)
		print(("[MavisDMA] Column relayout: available=%dpx col=%dpx gap=%dpx used=%dpx"):format(
			w, colWidth, COLUMN_GAP, colWidth * 2 + COLUMN_GAP
		))
	end

	RelayoutColumns()
	task.defer(RelayoutColumns)
	Columns:GetPropertyChangedSignal("AbsoluteSize"):Connect(RelayoutColumns)
