...
local POPPINS_ASSET = "rbxassetid://11702779409"

local function PoppinsFont(weight)
	local ok, font = pcall(function()
		return Font.new(POPPINS_ASSET, weight, Enum.FontStyle.Normal)
	end)
	if ok then
		print(("[MavisDMA] Font loaded OK: Poppins weight=%s family=%s"):format(tostring(weight), tostring(font.Family)))
		return font
	end
	warn("[MavisDMA] Failed to load Poppins font (" .. tostring(font) .. "), falling back to Gotham/Montserrat")
...
function Library:AddTab(config)
	...
	for i = 1, 2 do
		local col = New("Frame", {
			...
			Position = UDim2.new(i == 1 and 0 or 0.5, 0, 0, 0),
			AutomaticSize = Enum.AutomaticSize.Y,
			BackgroundTransparency = 1,
			LayoutOrder = i,
			Parent = Columns,
		})
		Pad(col, i == 1 and 0 or 8, i == 1 and 8 or 0, 0, 0)
		New("UIListLayout", {
