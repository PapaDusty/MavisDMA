...
-- Provides Window, Tab, Section, and Control (Toggle, Slider, Dropdown) classes.
-- All UI elements are fully responsive and customizable.
...
-- Theme defaults (can be overridden via Library.SetTheme)
local Theme = {
...
function Window.new(title, parent)
    ...
    -- Shadow
    local shadow = Create("ImageLabel", self.Main, {
        BackgroundTransparency = 1,
        Image = "rbxassetid://1316045048", -- shadow
    ...
    if player then
        local avatar = Create("ImageLabel", self.TopBar, {
            BackgroundTransparency = 1,
        ...
        local nameLabel = Create("TextLabel", self.TopBar, {
            BackgroundTransparency = 1,
            Text = player.Name,
            TextColor3 = Theme.Text,
            TextSize = 16,
    ...
    -- Content container (where tabs show their content)
...
function Window:CreateTab(name)
    local tab = {
        ...
        Buttons = {},
        Sections = {},
        Content = Create("ScrollingFrame", self.ContentContainer, {
            BackgroundTransparency = 1,
    ...
    -- Position buttons automatically (simplified)
...
function Section:UpdateLayout()
    ...
    -- Update canvas of scrolling frame
    local parent = self.ParentTab.Content
    parent.CanvasSize = UDim2.new(0, 0, 0, self.Frame.Position.Y.Offset + self.Frame.Size.Y.Offset + 20)
end

-- Control base (not instantiated directly)
local Control = {}
...
function Dropdown.new(parent, label, options, default)
    ...
    -- Close dropdown on clicking outside (simplified)
...
-- Section creator in tab
function Window.Tab:CreateSection(title)
    local section = Section.new(self, title)
    table.insert(self.Sections, section)
    -- Update layout after adding section
    self:UpdateSections()
    return section
end

function Window.Tab:UpdateSections()
    local y = 0
